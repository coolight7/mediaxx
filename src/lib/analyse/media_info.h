...
namespace mediaxx {
    ...
    public:
        ...
        MediaInfoEntity_c(const std::string_view in_filepath, const char** in_log) :
            mediaxx::AnalyseLogItem_c(in_log),
    ...
    public:
                    ...
                    case AVMEDIA_TYPE_VIDEO:
                        {
                            ...
                            result.append_comma();
                            result.append_key_value<"framerate">(fmt::format(
                                "{}/{}",
        ...
        bool saveFrameAsJPEG(
            ...
            do {
                ...
                if (ret != 0) {
                    item.setLog(
                        fmt::format("无法打开JPEG编码器: {}/{}", ret, utilxx::av_err2str(ret))
                    );
                    result = false;
                    break;
                }
                ...
                if (ret != 0) {
                    item.setLog(
                        fmt::format("发送帧到编码器失败: {}/{}", ret, utilxx::av_err2str(ret))
                    );
                    result = false;
                    break;
                }
                ...
                if (ret != 0) {
                    item.setLog(
                        fmt::format("从编码器接收包失败: {}/{}", ret, utilxx::av_err2str(ret))
                    );
                    result = false;
                    break;
                }
